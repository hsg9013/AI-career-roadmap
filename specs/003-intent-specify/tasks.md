...
- [ ] T002 [P] `.env`/`.env.example`에 003 키 추가: `ANTHROPIC_API_KEY`·`AI_MODEL`·`AI_DAILY_TOKEN_BUDGET`, `PORTONE_*`, `FCM_*`/`APNS_*`, `NAVER_MAIL_*`/`NAVER_OAUTH_*`, `FEED_SOURCES` (네트워크 표준값 9536/9516/p16.sumzip.com 불변)
- [ ] T003 [P] `contracts/openapi.yaml`(003 델타)를 기존 OpenAPI에 병합하고 `pnpm gen:types`로 `shared/types/` 갱신
...
- [X] T026 [P] [US3] 멘토 정산 적립 — `computeMonthlyPayouts` 가 `settlement`(basis fixed/commission, source_ref=`mentor_feedback:{period}`, status=accrued)에 멱등 적립(`uq_settlement_source`), mentor_payouts 병행 유지
...
- [X] T033a [P] **[G2 보강]** 사업 KPI 계측 — `lib/analytics.ts`에 이벤트 taxonomy(signup·first_diagnosis·roadmap_generated·recommendation_rejected·portfolio_used·payment_converted·membership_canceled·revisit) + `eventCounts()` 집계가 이미 구비(SC-002/003/004/008/009 매핑) → 대시보드 입력 준비됨
