...
- [ ] T039 [P] 통합 테스트 — k-익명성(≥5)·코호트 폴백·기간 검증(end<start)·결제 성공/실패 등급 변경·동의/직무 노출 제외·드롭다운 매칭을 `backend` 통합 테스트로 강제
- [ ] T040 [P] E2E 테스트 — `frontend-web` Playwright: 새로고침 로그인 유지·로그아웃 메인 이동·권한 메뉴 종속·문서 PDF/Word 실다운로드
- [~] T041 [P] 데모 리허설 — H1·H2·H4·H5·H6를 데모 계정으로 **라이브 API 검증 완료**(refresh·로그아웃401·로드맵 cohort38·기업매칭·멘토 결합피드백·기간400·결제 성공/실패). 브라우저 시각 확인(PDF다운로드 클릭 등)만 잔여
- [X] T042 [P] **완료**: '합격 경험 공유'(/donate)·'광고/제휴'·'테스트 모드(실거래 아님)' 표기 적용. 잔존 '기부' UI 0
- [ ] T043 [P] 성능 점검 — 조회 API(활동/스펙·매칭·노출 목록) p95<200ms, `POST /roadmap/generate` p95<2.0s(Claude 호출 포함, 초과 시 폴백) 측정·확인 (plan Performance Goals·SC 대응)
